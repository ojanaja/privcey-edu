            ...
            {showForm && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mb-6">
                    <div className="admin-card p-6">
                        <h2 className="text-lg font-semibold text-foreground mb-4">Buat Try Out Baru</h2>
                    ...
                    >
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                <Badge variant={tryout.is_active ? 'success' : 'danger'}>{tryout.is_active ? 'Aktif' : 'Off'}</Badge>
                                <button
                    ...
                    {showQuestionForm && (
                        <div className="admin-card p-6 mb-4">
                            <form onSubmit={handleAddQuestion} className="space-y-3">
                                <textarea required value={questionForm.question_text} onChange={(e) => setQuestionForm({ ...questionForm, question_text: e.target.value })} className="admin-input w-full px-3 py-2 text-sm h-20 resize-none" placeholder="Pertanyaan..." />
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <select value={questionForm.correct_answer} onChange={(e) => setQuestionForm({ ...questionForm, correct_answer: e.target.value as any })} className="admin-input px-3 py-2 text-sm">
                                        {['A', 'B', 'C', 'D', 'E'].map((o) => <option key={o} value={o}>Jawaban: {o}</option>)}
                                    </select>
                                </div>
                                <div className="flex gap-3 justify-end">
                                    <Button variant="ghost" size="sm" onClick={() => setShowQuestionForm(false)}>Batal</Button>
